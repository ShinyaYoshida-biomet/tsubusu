...
- When asked to implement a GitHub issue, complete the full workflow by creating or switching to a dedicated branch, implementing the change, running relevant validation, committing only in-scope changes, pushing the branch, and opening a pull request.
- Use an English issue or pull request description when the repository task is requested in English.
- Preserve unrelated local changes and do not include them in the issue commit or pull request.
